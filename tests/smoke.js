...
async function main() {
  ["src/server.js", "src/messaging.js", "public/admin.js", "public/app.js", "public/album.js"].forEach(file => {
    execFileSync(process.execPath, ["--check", path.join(root, file)], {stdio: "inherit"});
  });
  const testEnv = {...process.env, STORAGE_ROOT: testStorage};
  ...
  const productionCount=execFileSync(process.execPath,["-e","const db=require('./src/db');process.stdout.write(String(db.prepare('SELECT COUNT(*) total FROM events').get().total));"],{
    cwd:root,env:{...process.env,NODE_ENV:"production",STORAGE_ROOT:productionStorage},encoding:"utf8"
  });
  ...
  assert.throws(()=>execFileSync(process.execPath,[path.join(root,"src/seed.js")],{
    cwd:root,env:{...process.env,NODE_ENV:"production",STORAGE_ROOT:productionStorage},stdio:"pipe"
  }),/Command failed/);
  assert.throws(()=>execFileSync(process.execPath,[path.join(root,"src/server.js")],{
    cwd:root,env:{...process.env,NODE_ENV:"production",SITE_URL:"https://evento.example",STORAGE_ROOT:productionStorage},stdio:"pipe",timeout:5000
  }),/Command failed/);
  ...
  assert.equal(health.data.version, "6.14.1-rc.1");
  ...
  const registration=await request("/api/auth/register",{method:"POST",json:{displayName:"Cliente Registro Smoke",email:`registro-${process.pid}@example.test`,password:"RegistroSeguro123!",planCode:"starter",acceptTerms:true}});
  ...
  assert.ok(registeredEvents.data.some(item=>item.id===registration.data.eventId&&!item.published));
  ...
  const cookieLogout=await request("/api/auth/logout",{method:"POST",headers:{Cookie:cookie}});
  ...
  const event = eventList.data.find(item => item.slug === "francisco-ariana");
  ...
  try {
    ...
    assert.deepEqual(
      ...
      {starter: 69900, basic: 109900, premium: 179900, studio: 349900}
    );
    ...
    assert.equal(hiddenWhatsapp.label,undefined,"El catálogo comercial interno no debe exponerse al cliente.");
    ...
    assert.equal(transferAccount.response.status,200,JSON.stringify(transferAccount.data));
    ...
    const publicUpload = await request("/api/config/francisco-ariana");
    ...
    const publicSpotify = await request("/api/config/francisco-ariana");
    ...
    for(const mode of ["planned","confirmed"]){
      ...
      assert.ok(seatingPdfBytes.length>8000,`El plano ${mode} está incompleto.`);
      assert.match(seatingPdfBytes.toString("latin1"),/\/MediaBox \[0 0 792 612\]/);
      assert.match(seatingPdfBytes.toString("latin1"),/\/FontFile2/,"El plano debe incrustar fuentes.");
    ...
    const png=Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Y9WlWQAAAAASUVORK5CYII=","base64");
    photoForm.append("photos",new Blob([png],{type:"image/png"}),"recuerdo.png");
    photoForm.append("eventSlug","francisco-ariana");
    ...
    duplicatePhotoForm.append("eventSlug","francisco-ariana");
    ...
    emptyMessageForm.append("eventSlug","francisco-ariana");
    ...
    longMessageForm.append("eventSlug","francisco-ariana");
    ...
    const uploadedPhoto=photoRows.data.find(item=>item.batch_id===photoUpload.data.batchId);
    ...
    const publicMessages=await request("/api/public/photo-messages/francisco-ariana");
    ...
    invalidPhotoForm.append("eventSlug","francisco-ariana");
    ...
    tooManyPhotosForm.append("eventSlug","francisco-ariana");
    ...
    for(const physicalTemplate of ["auto-theme","coordinated-arch","editorial-silk","classic-frame"]){
      ...
      assert.match(physicalBytes.toString("latin1"),/\/FontFile2/,"La invitación física debe incrustar sus fuentes de imprenta.");
    ...
    for(const themeId of ["lavender-couture","cinematic-vows","storybook-seal","botanical-scroll"]){
      const updateTheme=await request("/api/admin/settings",{method:"PUT",token:clientToken,eventId,json:{themeId,physicalInvitation:{templateId:"auto-theme"}}});
      assert.equal(updateTheme.response.status,200);
      const themedInvite=await request(`/api/admin/physical-invitation.pdf?guestId=${realIds[0]}&template=auto-theme`,{token:clientToken,eventId});
      const themedBytes=Buffer.from(themedInvite.data);
      assert.equal(themedInvite.response.status,200,`No se generó la invitación física de ${themeId}.`);
      assert.ok(themedBytes.length>5000);
      if(qrOutputDir){fs.mkdirSync(qrOutputDir,{recursive:true});fs.writeFileSync(path.join(qrOutputDir,`invitacion-fisica-auto-${themeId}.pdf`),themedBytes);}
    }
    await request("/api/admin/settings",{method:"PUT",token:clientToken,eventId,json:{themeId:initialSettings.themeId}});
    ...
    for (const template of templates.data) {
      const pdf = await request(`/api/admin/qr-card.pdf?table=Mesa%201&template=${encodeURIComponent(template.id)}`, {
        token: clientToken, eventId
      });
      const bytes = Buffer.from(pdf.data);
      assert.equal(pdf.response.status, 200, `No se generó ${template.id}.`);
      assert.ok(bytes.subarray(0, 4).equals(Buffer.from("%PDF")), `${template.id} no devolvió un PDF.`);
      assert.ok(bytes.length > 5000, `${template.id} generó un PDF incompleto.`);
      assert.match(bytes.toString("latin1"), new RegExp(`/MediaBox \\[0 0 ${expectedMediaBoxes[template.format]}\\]`));
      assert.match(bytes.toString("latin1"),/\/FontFile2/,`${template.id} debe incrustar fuentes para impresión estable.`);
      if (qrOutputDir) {
        fs.mkdirSync(qrOutputDir, {recursive: true});
        fs.writeFileSync(path.join(qrOutputDir, `${template.id}.pdf`), bytes);
      }
    }
    ...
    const persistedMessages=await request("/api/public/photo-messages/francisco-ariana");
    assert.ok(persistedMessages.data.some(item=>item.message.includes("Gracias por compartir")));
    const persistedMusic=await request("/api/config/francisco-ariana");
  ...
  console.log("✓ Pruebas funcionales 6.14.1-rc.1 completadas");
}
