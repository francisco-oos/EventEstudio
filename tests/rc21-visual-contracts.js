...
for(const id of ["newspaper-fold","vintage-parchment","olive-universe-orbit","blue-aurora-reveal","botanical-cosmos-orbit"]){const opening=experiences.openings.find(item=>item.id===id);assert.ok(opening,`Falta ${id}.`);assert.equal(opening.renderer,"css");assert.match(styles,new RegExp(`\\.opening-${id}`));}
...
assert.match(app,/'night-flower-original':\{replay:6800,normal:6200\}/);assert.match(admin,/experienceOptions\(\)\.openings/);assert.match(app,/function renderOpeningSeal\(/);assert.match(app,/EventStudioWaxSeal/);assert.match(read("public/seal-renderer.js"),/function svgString\(/);assert.match(read("public/seal-renderer.js"),/function monogram\(/);
...
const unifiedTiming=/'unified-envelope':\{replay:(\d+),normal:(\d+)\}/.exec(app);assert.ok(unifiedTiming);assert.ok(Number(unifiedTiming[2])>=4200);
...
for(const id of ["previewOpeningBtn","storePreviewReplay","simulateCartBtn","phonePreviewBtn","openStationeryStudioBtn"]){assert.ok(adminHtml.includes(`id="${id}"`));assert.ok(admin.includes(`$('${id}')`),`${id} no está enlazado en admin.js.`);}
