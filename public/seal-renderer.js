(function(global){
  ...
  function svgString(definition={},context={},catalog={}){
    ...
    return `<svg class="wax-seal-svg" viewBox="0 0 500 500" xmlns="${NS}" role="img" aria-label="Sello de cera ${escapeXml(center)}"><defs><radialGradient id="${ids}-wax" cx="30%" cy="23%" r="84%"><stop offset="0%" stop-color="${shift(mat.light,16)}"/><stop offset="19%" stop-color="${mat.light}"/><stop offset="58%" stop-color="${mat.mid}"/><stop offset="86%" stop-color="${shift(mat.dark,-12)}"/><stop offset="100%" stop-color="${mat.dark}"/></radialGradient><radialGradient id="${ids}-gloss" cx="36%" cy="28%" r="48%"><stop offset="0%" stop-color="#ffffff" stop-opacity=".34"/><stop offset="42%" stop-color="#ffffff" stop-opacity=".08"/><stop offset="100%" stop-color="#ffffff" stop-opacity="0"/></radialGradient><filter id="${ids}-shadow" x="-50%" y="-50%" width="200%" height="200%"><feDropShadow dx="0" dy="9" stdDeviation="12" flood-color="#0a0704" flood-opacity=".46"/></filter><filter id="${ids}-bevel" x="-40%" y="-40%" width="180%" height="180%" color-interpolation-filters="sRGB"><feGaussianBlur in="SourceAlpha" stdDeviation="1.6" result="blur"/><feDiffuseLighting in="blur" surfaceScale="${surfaceScale}" diffuseConstant=".9" lighting-color="#fff3d6" result="diffuse"><feDistantLight azimuth="235" elevation="52"/></feDiffuseLighting><feComposite in="diffuse" in2="SourceAlpha" operator="in" result="diffuseClip"/><feBlend in="SourceGraphic" in2="diffuseClip" mode="multiply" result="shaded"/><feSpecularLighting in="blur" surfaceScale="${surfaceScale}" specularConstant="${spec}" specularExponent="18" lighting-color="#fff8e9" result="spec"><feDistantLight azimuth="235" elevation="58"/></feSpecularLighting><feComposite in="spec" in2="SourceAlpha" operator="in" result="specClip"/><feComposite in="specClip" in2="shaded" operator="arithmetic" k1="0" k2="1" k3="1" k4="0"/></filter>${grain}<clipPath id="${ids}-clip"><path d="${blobPath(def.borderStyle,context.seed||center)}"/></clipPath><path id="${ids}-top" d="M 128 250 A 122 122 0 0 1 372 250"/><path id="${ids}-bottom" d="M 128 250 A 122 122 0 0 0 372 250"/></defs><path d="${blobPath(def.borderStyle,context.seed||center)}" fill="url(#${ids}-wax)" filter="url(#${ids}-shadow)"/><path d="${blobPath(def.borderStyle,context.seed||center)}" fill="url(#${ids}-gloss)" opacity="${full?.58:.42}"/><ellipse cx="205" cy="168" rx="86" ry="48" transform="rotate(-18 205 168)" fill="#fff" opacity="${full?.12:.075}" clip-path="url(#${ids}-clip)"/>${texture}<g filter="url(#${ids}-bevel)" fill="${detail}" stroke="${detail}"><circle cx="250" cy="250" r="166" fill="none" stroke-width="3.4"/><circle cx="250" cy="250" r="150" fill="none" stroke-width="2.2"/>${ornamentMarkup(def.ornament,detail)}<text x="250" y="${250+Number(def.verticalOffset||0)}" text-anchor="middle" dominant-baseline="middle" font-family="${escapeXml(fontFamily)}" font-size="${def.fontSize}" letter-spacing="${def.kerning}" fill="${detail}" stroke="none">${escapeXml(center)}</text>${top?`<text font-family="${escapeXml(fontFamily)}" font-size="15" letter-spacing="2.4" fill="${detail}" stroke="none"><textPath href="#${ids}-top" startOffset="50%" text-anchor="middle">${top}</textPath></text>`:""}${bottom?`<text font-family="${escapeXml(fontFamily)}" font-size="15" letter-spacing="2.4" fill="${detail}" stroke="none"><textPath href="#${ids}-bottom" startOffset="50%" text-anchor="middle">${bottom}</textPath></text>`:""}</g></svg>`;
  }
